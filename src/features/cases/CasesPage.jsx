...
// skeleton barrels
import { Button, Drawer, ConfirmDialog, Loader, useToast } from "@/components/common";
...
export default function CasesPage() {
  ...
  // form model
  const emptyCase = { name: "", description: "", clientId: "", status: "open", assignedUsers: [], primaryLawyerId: "" };
  ...
  const rows = useMemo(() => (Array.isArray(list) ? list : []).map((c) => ({
    ...
    primaryLawyerId: c.primaryLawyerId || "",
    _raw: c,
  })), [list]);
  ...
  const filtered = useMemo(() => {
    ...
    if (q) r = r.filter((x) => String(x.name).toLowerCase().includes(q) || String(x.description).toLowerCase().includes(q) || String(x.clientId).toLowerCase().includes(q));
  ...
  const columns = [
    ...
    { id: "name", header: "Case", accessor: (r) => (
      <button className="text-[color:var(--lb-primary-700)] hover:underline" onClick={() => openEdit(r)}>{r.name}</button>
    ), sortable: true, width: 260 },
    { id: "clientId", header: "Client ID", accessor: (r) => r.clientId, sortable: true, width: 180 },
    { id: "status", header: "Status", accessor: (r) => cap(r.status), sortable: true, width: 120 },
    { id: "assignedUsers", header: "Assigned", accessor: (r) => r.assignedUsers?.join(", ") || "—", width: 260 },
  ];
  ...
  function openEdit(row) {
    ...
    setForm({
      ...
      assignedUsers: Array.isArray(row.assignedUsers) ? row.assignedUsers : [],
      primaryLawyerId: row.primaryLawyerId || "",
    });
    setOpen(true);
  }
