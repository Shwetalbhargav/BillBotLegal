...
import { fetchEmails, addEmail } from "@/store/emailSlice";
import { pushClio } from "@/store/emailSlice";
...
export default function EmailEntriesPage() {
  ...
  const [mode, setMode] = useState("create"); // create | view (edit pending backend)
 ...
 sortable: false,
  ...
  title={mode === "create" ? "New Email Entry" : "Email Details"}
>
  {mode === "view" ? (
    <div className="space-y-3">
      ...
      {/* Action buttons moved to the bottom, consistent with create mode */}
...
function uiToPayload(f){
  return {
    ...
    date: f.date || null,
    status: f.status,
    summary: e.billableSummary || e.summary || e.description || "",
  };
}
