...
// Skeleton components (adjust paths if your aliases differ)
 ...
 * - Focused on business done by Partners & Lawyers (interns excluded by default)
 * - Group by Attorney/Client/Case/Date
 * - Case status joined from caseSlice
 * - KPIs: Hours, Revenue, Avg Rate, Logged %
 * - INR formatting
 */
export default function AnalyticsPage() {
  ...
  const [filters, setFilters] = useState({
    q: "",
    groupBy: "user",             // default to Attorney
    from: "",
    to: "",
    roles: ["Partner", "Lawyer"],// Partners & Lawyers only
    excludeInterns: true,
    caseStatus: "ALL",
  });
  ...
  // Build a status lookup from caseSlice
  ...
  // 1) normalize entries; 2) annotate with status; 3) filter by role/intern/status/query/date
  ...
  const filtered = useMemo(() => {
    ...
    rows = rows.filter((r) => {
      ...
      // If no role info, keep the row (don't drop data)
      if (!role) return true;

      // If roles are selected, require a match
      if (filters.roles?.length) {
        return filters.roles.some(sel => role.includes(sel.toLowerCase()));
      }
      return true;
    });
    ...
    if (filters.caseStatus && filters.caseStatus !== "ALL") {
      rows = rows.filter((r) => (r.caseStatus || "Unknown") === filters.caseStatus);
    }
  ...
  // group/aggregate
  const grouped = useMemo(() => groupRows(filtered, filters.groupBy), [filtered, filters.groupBy]);
  ...
  // KPIs
  const kpis = useMemo(() => {
    const hours = filtered.reduce((s, r) => s + (r.hours || 0), 0);
    const revenue = filtered.reduce((s, r) => s + (r.revenue || 0), 0);
    const avgRate = hours ? revenue / hours : 0;
    const loggedPct = avgPct(filtered.map((r) => r.loggedPct).filter((v) => isFinite(v)));
  ...
  return (
    <div className="lb-reset p-6">
      ...
      <TableToolbar>
        <Input
          placeholder="Search client/case/user…"
          value={filters.q}
          onChange={(e) => { setPage(1); setFilters((f) => ({ ...f, q: e.target.value })); }}
        />
        <Select
          value={filters.groupBy}
          onChange={(e) => { setPage(1); setFilters((f) => ({ ...f, groupBy: e.target.value })); }}
        >
        ...
        {/* Role filter */}
        <Select
          value={JSON.stringify(filters.roles)}
          onChange={(e) => {
            setPage(1);
            const val = JSON.parse(e.target.value);
            setFilters((f) => ({ ...f, roles: val }));
          }}
        >
        ...
        <Select
          ...
          onChange={(e) => { setPage(1); setFilters((f) => ({ ...f, caseStatus: e.target.value })); }}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <KpiCard label="Total Hours" value={fmtNumber(kpis.hours)} loading={loading} />
        <KpiCard label="Revenue" value={fmtCurrency(kpis.revenue)} loading={loading} />
        <KpiCard label="Avg Rate" value={fmtCurrency(kpis.avgRate)} loading={loading} />
        <KpiCard label="Logged %" value={fmtPercent(kpis.loggedPct)} loading={loading} />
      </div>
...
function groupRows(rows, groupBy) {
  const map = new Map();
  const keyOf = (r) => {
    if (groupBy === "client") return r.client || "—";
    if (groupBy === "case") return r.case || "—";
    if (groupBy === "user") return r.user || "—";
    return r.date ? new Date(r.date).toISOString().slice(0, 10) : "—"; // date
  };
  ...
  for (const r of rows) {
    ...
    if (isFinite(r.rate)) { prev.sumRate += r.rate; prev.rateDen += 1; }
    if (isFinite(r.loggedPct)) { prev.sumPct += r.loggedPct; prev.pctDen += 1; }
    map.set(k, prev);
  }
...
function buildColumns(groupBy) {
  const first = groupBy === "client" ? "Client" : groupBy === "case" ? "Case" : groupBy === "user" ? "Attorney" : "Date";
  return [
