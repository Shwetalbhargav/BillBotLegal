...
export default function EmailEntriesPage({
  ...
  // drawer state (rename to avoid prop collision)
  ...
  const rows = useMemo(
    () =>
      (Array.isArray(list) ? list : []).map((e) => ({
        ...
        minutes: Number(
          e.typingTimeMinutes ?? e.durationMinutes ?? e.minutes ?? e.duration ?? 0
        ),
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.status)
      r = r.filter((x) => String(x.status).toLowerCase() === filters.status);
